...
textures = {
			...
			CLOUD1 : pygame.image.load('H:/pygame/Minecraft.pngs/cloud1.png')
			}
...
INVFONT = pygame.font.Font('Thy textsy stuff.ttf', 18)
...
while True:
	...
	DISPLAYSURF.blit(textures[BIRB].convert_alpha(),(birdx,birdy))
	birdx+=6
	if birdx > MAPWIDTH*TILESIZE:
		birdy= random.randint(0,MAPHEIGHT*TILESIZE)
		birdx = -200
		
	DISPLAYSURF.blit(textures[CLOUD1].convert_alpha(),(lcloudx,lcloudy))
	lcloudx-=3
	if lcloudx < -200:
		lcloudy= random.randint(0,MAPHEIGHT*TILESIZE)
		lcloudx = MAPWIDTH*TILESIZE
